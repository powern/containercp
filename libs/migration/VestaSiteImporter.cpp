...
VestaSiteImporter::ImportSqlResult VestaSiteImporter::import_sql(const Options& opts) {
    ...
    // Add trusted proxy block for HTTPS detection behind central nginx proxy
    {
        ...
        if (existing.find("BEGIN CONTAINERCP TRUSTED PROXY") == std::string::npos) {
            const std::string wp_settings = "require_once ABSPATH . 'wp-settings.php';";
            auto wp_settings_pos = existing.find(wp_settings);
            if (wp_settings_pos != std::string::npos) {
                existing.insert(wp_settings_pos, proxy_block + "\n");
            } else {
...
VestaSiteImporter::UpgradeResult VestaSiteImporter::upgrade_site(const Options& opts) {
    ...
    if (!wp_config.empty()) {
        ...
        if (content.find("BEGIN CONTAINERCP TRUSTED PROXY") == std::string::npos) {
            ...
            const std::string wp_settings = "require_once ABSPATH . 'wp-settings.php';";
            auto wp_settings_pos = content.find(wp_settings);
            if (wp_settings_pos != std::string::npos) {
                content.insert(wp_settings_pos, proxy_block + "\n");
            } else {
