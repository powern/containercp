...
core::OperationResult DockerComposeProvider::create_site(site::Site& site, core::ProgressCallback progress) {
    ...
    fs_.create_file(site_dir + "config/mariadb/initdb/10-containercp-service-account.sh",
        ...
        "GRANT SELECT, INSERT, UPDATE, DELETE, CREATE, DROP, INDEX, ALTER, CREATE TEMPORARY TABLES, LOCK TABLES, SHOW VIEW, CREATE VIEW, TRIGGER, EVENT, CREATE ROUTINE, ALTER ROUTINE, EXECUTE, REFERENCES, GRANT OPTION ON \\`$MYSQL_DATABASE\\`.* TO '$CONTAINERCP_DB_SERVICE_USER'@'%';\n"
        "FLUSH PRIVILEGES;\n"
        "SQL\n");
