...
core::OperationResult AcmeClient::load_or_create_account(const std::string& key_path) {
    logger_.info("ACME", "load_or_create_account");
    account_key_path_ = key_path;
    logger_.info("ACME", "2: key_path=" + key_path);
    // Check if key exists
    std::ifstream key_file(key_path);
    bool exists = key_file.good();
    key_file.close();
    logger_.info("ACME", "3: key exists=" + std::to_string(exists));
    if (!exists) {
        logger_.info("ACME", "Generating new account key");
    ...
    EVP_PKEY* pkey = PEM_read_bio_PrivateKey(key_bio, nullptr, nullptr, nullptr);
    BIO_free(key_bio);
    logger_.info("ACME", std::string("6: after PEM_read, pkey=") + (pkey ? "valid" : "null"));
    if (!pkey) {
    ...
    logger_.info("ACME", "9: after sign_jws, jws.size=" + std::to_string(jws.size()));
    HttpResponse post_resp = http_post(new_account_url_, jws, "application/jose+json");
    logger_.info("ACME", "11: after http_post status=" + std::to_string(post_resp.status_code)
    ...
    logger_.info("ACME", "12b: account failed, error=" + error_msg);
...
core::OperationResult AcmeClient::create_order(const std::vector<std::string>& domains, Order& order) {
    ...
    logger_.info("ACME", "newOrder status=" + std::to_string(resp.status_code)
                 + " location=" + resp.location + " body=" + resp.body);
    order.url = resp.location;
    logger_.info("ACME", "order.url=" + order.url);
    // Parse response body
...
core::OperationResult AcmeClient::get_authorization(const std::string& authz_url, Authorization& authz) {
    ...
    logger_.info("ACME", "authz body: " + resp.body);
    authz.url = authz_url;
    ...
    logger_.info("ACME", "authz.identifier.value=" + authz.domain);
...
core::OperationResult AcmeClient::finalize_order(const std::string& finalize_url, const std::string& order_url, const std::string& csr_pem, std::string& cert_url) {
    logger_.info("ACME", "finalize: csr_pem.size=" + std::to_string(csr_pem.size())
    ...
    logger_.info("ACME", "finalize: POST " + finalize_url);
    auto resp = acme_post(finalize_url, payload);
    logger_.info("ACME", "finalize response: http=" + std::to_string(resp.status_code)
                 + " body=" + resp.body);
    if (resp.status_code != 200) {
    ...
    if (order_status == "processing") {
        ...
        for (int i = 0; i < 30; ++i) {
            ...
            logger_.info("ACME", "poll #" + std::to_string(i+1)
