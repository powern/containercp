...
#define RD_PROFILE(db, p) ( \
    p.id=static_cast<uint64_t>(db.column_int(0)), p.profile_name=db.column_text(1), \
    p.type=profile::profile_type_from_string(db.column_text(2)), \
...
#define RD_AG(db, g) ( \
    ...
    g.name=std::to_string(g.access_user_id)+"-"+std::to_string(g.site_id), true)
#define RD_MD(db, m) ( \
    ...
    m.dkim_public_key_dns=db.column_text(8), \
    m.max_mailboxes=static_cast<uint64_t>(db.column_int(9)), m.max_aliases=static_cast<uint64_t>(db.column_int(10)), \
    m.catch_all=db.column_text(11), m.enabled=(db.column_int(12)!=0), \
    m.created_at=db.column_text(13), m.updated_at=db.column_text(14), m.name=m.domain_name, true)
...
#define RD_MA(db, a) (a.id=static_cast<uint64_t>(db.column_int(0)), a.domain_id=static_cast<uint64_t>(db.column_int(1)), a.source_local_part=db.column_text(2), a.destination=db.column_text(3), a.enabled=(db.column_int(4)!=0), a.created_at=db.column_text(5), a.updated_at=db.column_text(6), a.name=a.source_local_part, true)
    ...
    static bool read_bool_col(SQLiteDB& db, int col, bool& out) {
        out = (db.column_int(col) != 0); return true;
    }
    static bool read_string_col(SQLiteDB& db, int col, std::string& out) {
