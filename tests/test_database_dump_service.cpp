...
struct DumpFakeProvider : database::DatabaseProvider {
    ...
    database::DatabaseProviderResult create_temporary_sql_console_user(const database::MariaDBConnectionTarget&, const database::DatabaseProviderCredential&, const std::string&, const std::string&, const std::string&) const override { return ok("temporary_sql_console_user_ready"); }
    database::DatabaseProviderResult drop_temporary_sql_console_user(const database::MariaDBConnectionTarget&, const database::DatabaseProviderCredential&, const std::string&, const std::string&) const override { return ok("temporary_sql_console_user_dropped"); }
...
TEST_CASE("DB-4 real MariaDB export and import restores expected rows in disposable stack") {
    ...
    std::ofstream(initdb / "10-containercp-service-account.sh")
        ...
        << "GRANT SELECT, INSERT, UPDATE, DELETE, CREATE, DROP, INDEX, ALTER, CREATE TEMPORARY TABLES, LOCK TABLES, SHOW VIEW, CREATE VIEW, TRIGGER, EVENT, CREATE ROUTINE, ALTER ROUTINE, EXECUTE, REFERENCES, GRANT OPTION ON \\`db4_real_db\\`.* TO '$CONTAINERCP_DB_SERVICE_USER'@'%';\n"
        << "SQL\n";
