...
## Phase 9 — Baseline capture and MailConfigResult

### ResourceBaseline

Before each import, `LegacyImporter::capture_baseline(type)` captures
the current SQLite state using `SQLiteSnapshotReader` (checked typed
loads). The baseline is stored in `ImportResult::baseline`:
...
Baseline capture failure stops import before parsing or writing with
`baseline_capture_failed`. Canonical checksums match Verification's
format exactly via `StorageCanonicalizer`.

### MailConfigResult (presence-aware)

`LegacyDatasetReader::read_mail_config()` returns:
...
### Shared parser identity

`LegacyDatasetReader` is the single authoritative TXT parser for all
17 resource types. Both `LegacyImporter` and `Verification` consume
identical `DatasetResult` models from this reader.

Phase 9 (migration verification) is implemented in
`Verification.h/.cpp`.
Phase 10 (legacy archive) and Phase 11 (startup gate) are not
implemented.
