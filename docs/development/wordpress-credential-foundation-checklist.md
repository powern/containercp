...
### [x] WP-R2.2 Review metadata consistency
...
Result: Complete. Defined database credential metadata as a verified storage projection rather than an assumed cross-resource transaction. `DatabaseCredentialRotationAdapter` now receives a metadata password reader and verifies storage read-back after metadata persistence before reporting success. If persistence fails or cannot be verified, the adapter restores in-memory metadata to the old password and compensation verifies both in-memory metadata and the stored database record before reporting `compensated`. Partial storage updates or unreadable storage during rollback verification return `manual_recovery_required`; thrown metadata persistence/read-back errors are converted to fail-closed verification results. ServiceRegistry now verifies metadata through `Storage::load_databases()` without adding schema migration. Tests cover metadata write failure, persistence exceptions, successful write that cannot be read back, partial metadata update, read-back exceptions, rollback verification, and simulated daemon-restart consistency via separate stored metadata state. Validation passed with `cmake --build build-wp0 --target containercp_tests containercp containercpd -- -j1` and no compiler warnings, `build-wp0/tests/containercp_tests -tc="*DatabaseCredentialRotation*"` (`39` cases, `341` assertions), `*database*` (`63` cases, `546` assertions), `*WordPress*` (`64` cases, `379` assertions), full CTest (`1/1`), and `git diff --check`.

### [x] WP-R2.3 Review password transport
...
Result: Complete. `MariaDBCredentialProvider` keeps the length-framed stdin transport and no longer treats delimiter-like credential text as syntax. Identity values remain constrained to the reviewed MariaDB user/host alphabet, while password values now support valid imported WordPress/MariaDB credentials containing spaces, quotes, backslashes, semicolons, hashes, equals signs, brackets, tabs, newlines, carriage returns, non-ASCII bytes, and empty old passwords. Password values are escaped for MariaDB client option files and escaped separately for `ALTER USER` SQL literals. NUL, unsupported control bytes, DEL, and overlong values still fail closed before any secret file is written because they cannot be safely represented across both transport paths. Tests cover option-file escaping, empty imported password verification, SQL literal escaping, unsupported password rejection, redacted failures, and no secret argv exposure. Validation passed with `cmake --build build-wp0 --target containercp_tests containercp containercpd -- -j1` and no compiler warnings, `build-wp0/tests/containercp_tests -tc="*MariaDBCredentialProvider*"` (`16` cases, `104` assertions), plus final focused/full validation recorded in `CHANGELOG.md`.

### [x] WP-R2.4 Review shared user assessment
...
Result: Complete. Clean configure passed with `cmake -S . -B build-wp-r27 -G Ninja -DCMAKE_BUILD_TYPE=Release`. The first clean build command hit the 600s tool timeout while compilation was still progressing; rerunning `cmake --build build-wp-r27 --target containercp_tests containercp containercpd -- -j1` completed successfully with no compiler warnings. Full doctest passed from `build-wp-r27` (`789` cases, `5338` assertions). Focused suites passed for `*DatabaseCredentialRotation*` (`41` cases, `357` assertions), `*MariaDBCredentialProvider*` (`17` cases, `115` assertions), `*database*` (`65` cases, `562` assertions), `*WordPress*` (`64` cases, `379` assertions), `*API*` (`18` cases, `73` assertions), `*Command*` (`18` cases, `67` assertions), and `VestaSiteImporter*` (`31` cases, `79` assertions). Full CTest passed (`1/1`). `node --check web/app.js`, `build-wp-r27/containercp --version`, `build-wp-r27/containercpd --version`, and `git diff --check` passed. Static secret-surface scans found only expected auth password-change fields and the `db_password_present` boolean in Web/API surfaces; no WordPress database credential value exposure was found in Web UI/API/daemon/CLI surfaces. `gh` is unavailable and `.github/workflows` is absent, so no remote CI status is available. `git status --short` contains only untracked local validation build directories: `build-wp0/`, `build-wp-r11/`, and `build-wp-r27/`. No live site, live database, production credential, production `wp-config.php`, or production deployment was touched.

## WP-R3 Post-Mutation Context Preservation

Status: Complete. This workflow correction is limited to preserving rotation context during post-mutation state investigation after a failed MariaDB password-change command. It does not redesign the architecture, change ContainerCP concepts, deploy to production, or execute live credential rotation.

### [x] WP-R3.1 Review post-mutation verification flow

Objective: Verify that failed old-password verification after an uncertain MariaDB mutation cannot destroy context before new-password verification runs.

Result: Issue confirmed and fixed. `DatabaseCredentialRotationService` previously used the normal `verify_old_credential()` and `verify_new_credential()` dependencies while determining the real MariaDB state after a failed `change_mariadb_password()` command. The concrete `DatabaseCredentialRotationAdapter::verify_old_credential()` intentionally erased rotation context on failed old-credential verification, which meant the subsequent new-credential check could fail with `rotation_context_missing` even when `ALTER USER` had actually succeeded. Added explicit `probe_old_credential()` and `probe_new_credential()` dependency operations for this investigation window. The service now calls probes after failed mutation commands. The adapter probes verify credentials without erasing context, performing cleanup, or terminating the workflow. Normal `verify_old_credential()` cleanup behavior remains unchanged for pre-mutation verification and compensation failure paths.

### [x] WP-R3.2 Regression tests

Objective: Cover `ALTER USER` completed but command returned failure, old password invalid, new password valid, context preserved, compensation starts correctly, and the workflow reaches the expected state using real adapter behavior.

Result: Complete. Added a real-adapter regression where the fake MariaDB runner applies `newpass`, returns command failure after `ALTER USER`, rejects the old-password probe, accepts the new-password probe, then forces metadata persistence failure. The workflow records `mariadb_password_change_confirmed`, proceeds through WordPress/config/runtime verification, starts compensation, restores MariaDB to `oldpass`, restores `wp-config.php`, restores in-memory/stored metadata, and reaches `rotation_compensated`. Service-level tests now assert the post-mutation investigation calls `probe_old_credential` and `probe_new_credential` instead of cleanup-capable verification calls.

### [x] WP-R3.3 Final validation

Objective: Perform a clean build, complete test suite, WordPress credential rotation tests, database credential tests, compensation tests, and verify the post-mutation workflow.

Result: Complete. Clean configure/build passed with `cmake -S . -B build-wp-r3-clean -G Ninja -DCMAKE_BUILD_TYPE=Release` and `cmake --build build-wp-r3-clean --target containercp_tests containercp containercpd -- -j1` with no compiler warnings. Full doctest passed (`790` cases, `5356` assertions). Focused tests passed for `*DatabaseCredentialRotation*` (`42` cases, `375` assertions), `*database*` (`66` cases, `580` assertions), `*WordPress*` (`64` cases, `379` assertions), and `*MariaDBCredentialProvider*` (`17` cases, `115` assertions). Full CTest passed (`1/1`) and `git diff --check` passed. No live site, live database, production credential, production `wp-config.php`, or production deployment was touched.
