...
| 6 | **Authentication isolation ≠ sender identity isolation** | Per-site SASL credentials (Layer 2) prevent site A from authenticating as site B. Sender identity isolation (Layer 3) — restricting which `From:` addresses a site can use — is deferred to a future Postfix upgrade due to `@domain` wildcard incompatibility in Postfix 3.7.11. See 4.8.1 for the full security model. |
...
Mapped in Postfix (future): site-{SITE_ID}@php.containercp.internal → @{site_domain}
Note: The sender_login map is generated during enable_mail() but not activated in MVP
(Layer 3 deferred — see 4.8.1)
```
...
│  ┌───────────────────────────────────────────────────────┐  │
│  │ smtpd_client_*_rate_limit (global, per-IP)            │  │  Stage 2
│  │ Per-SASL-identity rate limiting (future)              │  │
│  └───────────────────────────────────────────────────────┘  │
├─────────────────────────────────────────────────────────────┤
│  Layer 4: Destination restrictions                         │
│  ┌───────────────────────────────────────────────────────┐  │
│  │ reject_unauth_destination — prevents open relay       │  │  Stage 2
│  │ Authenticated users can relay to any destination      │  │
│  └───────────────────────────────────────────────────────┘  │
├─────────────────────────────────────────────────────────────┤
...
│  ┌───────────────────────────────────────────────────────┐  │
│  │ Per-site SASL credentials (unique per site)           │  │  Stage 4-5
│  │ Site A CANNOT authenticate as Site B                  │  │
│  │ Site A CANNOT intercept Site B's mail                 │  │
│  └───────────────────────────────────────────────────────┘  │
├─────────────────────────────────────────────────────────────┤
...
# 3. SMTP AUTH tests
# 3a. Correct credentials → 235 Authentication successful, 250 queued
...
# 3c. No auth → rejected before MAIL FROM
...
# Expected: site-42@php.containercp.internal → @example.com (or similar)

# === P1: Config reload verification ===
# ⚠️ postconf -e FAILS on bind-mounted main.cf (Device or resource busy)
...
**Note:** `smtpd_sender_restrictions` uses `permit_sasl_authenticated` in MVP (no sender identity isolation — see 4.8.1). The `reject_sender_login_mismatch` restriction is NOT enabled in MVP. Sender_login map is generated for future use.
...
| `libs/mail/providers/DockerMailProvider.cpp` | Add SASL config (`smtpd_sasl_auth_enable`, `smtpd_sasl_type`, `smtpd_sasl_path`) to `write_postfix_config()`; add `smtpd_sender_login_maps` (generated but NOT activated — deferred Layer 3; see 4.8.1); add rate limits; add `php.containercp.internal` domain config; fix TLS cert SAN generation in `ensure_certificate()` | 2 | `postconf -n` shows new directives; `openssl x509` shows SAN |
...
| `libs/mail/SiteMailCredentials.h/.cpp` | **New**: per-site SASL credential management (generate, remove, find, apply, revoke). Also generates sender_login map entry per site (future use — Layer 3 deferred) | 5 | Unit test: credential generation + Postfix map entry |
