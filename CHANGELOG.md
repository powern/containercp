...
**Files changed:** `libs/access/FilesystemPermissionInspector.cpp`, `libs/access/LocalSftpProvider.cpp`, `libs/api/ApiServer.cpp`, `libs/core/ServiceRegistry.cpp`, `tests/test_access.cpp`, `CHANGELOG.md`
...
**Files changed:** `libs/access/SshdConfigWriter.h` (new), `libs/access/SshdConfigWriter.cpp` (new), `libs/access/SshdAuthorizedKeysWriter.h` (new), `libs/access/SshdAuthorizedKeysWriter.cpp` (new), `libs/access/LocalSftpProvider.h`, `libs/access/LocalSftpProvider.cpp`, `libs/core/ServiceRegistry.cpp`, `CMakeLists.txt`, `tests/CMakeLists.txt`, `tests/test_sshd_writers.cpp` (new), `tests/arch009_linux_integration.cpp`, `planning/ARCH-009-IMPLEMENTATION-PLAN.md`, `planning/project-status.md`, `CHANGELOG.md`

**User-visible behavior:** Daemon now manages per-user authorized_keys files with `restrict` prefix for SFTP-only access. Sshd include file is automatically installed when the first SFTP user is created. All operations are fully transactional with rollback.

**Validation:** Full unit suite passed. Full CTest passed with privileged integration PASS (or skip when marker absent). Cleanup verification proved no test identities, paths, mounts, or processes remain. `git diff --check` passed.

**Known risks:** Implementation targets Debian 12/13 only. Non-systemd hosts and non-Debian distributions are not supported. Multi-key revocation recovery after daemon crash and authorized_keys drift from external modification are not addressed.
...
**Files changed:** `libs/access/LocalSftpProvider.cpp`, `tests/test_access.cpp`, `docs/ARCH-009-PHASE3-ACCEPTANCE.md` (new), `planning/project-status.md`, `planning/ARCH-009-PHASE3-DESIGN.md`, `planning/ARCH-009-IMPLEMENTATION-PLAN.md`, `CHANGELOG.md`

**User-visible behavior:** Phase 3 is formally accepted with explicit residual risks. Startup reconciliation no longer self-blocks applying-grant cleanup during `Starting`. Pending grant revoke no longer succeeds after failed cleanup of stale membership/group state.

**Validation:** Focused ARCH-009 doctests passed (`197` cases, `680` assertions). Full unit suite passed via `ctest --test-dir build2 -R '^containercp_tests$' --output-on-failure`. Privileged integration passed (not skipped) via `CONTAINERCP_DISPOSABLE_TEST_HOST=1 CONTAINERCP_ARCH009_LINUX_INTEGRATION=1 ctest --test-dir build2 -L arch009_linux_integration --output-on-failure`. Full CTest passed with privileged opt-in (`2/2` tests passed). Cleanup verification proved no `au-arch46it` passwd entry, no `au-arch46it` group, no `ccp-arch009-it` group, no matching mounts, no `/srv/containercp/arch009-linux-integration` path, no marker file, no matching leaked process, and no SQLite test file. `git diff --check` passed.

**Known risks:** Real Linux privileged coverage for ACL, bind mount, unmount, grant lifecycle, and SQLite lifecycle remains deferred; these are accepted residual risks and must be covered before exposing real SFTP login. Final commit SHA is reported in the task response to preserve one logical Task48 commit.
...
**Files changed:** `CMakeLists.txt`, `libs/access/ManagedPathValidator.h` (new), `libs/access/ManagedPathValidator.cpp` (new), `libs/access/LocalSftpProvider.cpp`, `libs/access/SystemAccountCommandRunner.h`, `libs/access/SystemAccountCommandRunner.cpp`, `libs/core/ServiceRegistry.cpp`, `tests/CMakeLists.txt`, `tests/test_access.cpp`, `CHANGELOG.md`

**User-visible behavior:** Privileged commands now use canonical absolute paths (e.g., `/usr/bin/chgrp` instead of `chgrp`), eliminating PATH-based attacks. Path validation enforces containment within the managed root (`/srv/containercp/users`) with symlink detection. ACL specs are limited to the exact forms generated by ContainerCP. chmod accepts only octal values (3-4 digits). UID/GID values are validated against the configured managed ranges (10000-19999 for UID, 20000-29999 for GID).

**Validation:** Full doctest suite passed (1246 cases, ~7800 assertions, 0 failures). 7 new test cases with 146 assertions verify canonical paths, strict ACL grammar, strict octal mode, UID/GID range enforcement, and managed-path validation. `git diff --check` passed. Zero compiler warnings.

**Known risks:** The cross-user and cross-site path validation is performed at the `LocalSftpProvider` level, not in `SystemAccountCommandRunner`. This is by design — the provider has access to username and site identity information that the runner lacks. TOCTOU between path validation and command execution remains a theoretical risk; `execve` is used with immediate path resolution.
...
**Files changed:** `web/pages/migration.js`, `libs/api/ApiServer.cpp`, `CHANGELOG.md`

**User-visible behavior:** A completed migration now shows `Current stage` as `3`, the actual Site ID, `Files` as imported, and `SQL` as imported. Migration jobs no longer claim a mail configuration step.

**Validation:** Local build passed with `cmake --build build-db5 -j2`. Migration-focused doctests passed (`39` cases, `254` assertions). SSL-focused doctests passed (`19` cases, `100` assertions). Proxy-focused doctests passed (`6` cases, `48` assertions). Frontend syntax passed for `web/pages/migration.js` and `web/pages/ssl.js`; `git diff --check` and frontend baseline check passed.

**Known risks:** Mail still remains an explicit post-migration operator task; this change corrects UI/job reporting only and does not add mail import support.

---

## 2026-07-22 | `this commit` | SSL - Attach HTTPS proxy after certificate issue

**Summary:** Fixed SSL issuance finalization so a successfully issued certificate is immediately attached to the domain proxy configuration instead of only reloading nginx.

**Files changed:** `libs/api/ApiServer.cpp`, `CHANGELOG.md`

**User-visible behavior:** Ordering a certificate now enables the HTTPS vhost for the domain in the same SSL job. If proxy attachment fails after issuance, the job fails and `https_enabled` is reset instead of reporting an enabled certificate without a working HTTPS route.

**Validation:** Local build passed with `cmake --build build-db5 -j2`. SSL-focused doctests passed (`19` cases, `100` assertions). Proxy-focused doctests passed (`6` cases, `48` assertions). Frontend syntax passed for `web/pages/ssl.js`; `git diff --check` and frontend baseline check passed.

**Known risks:** This changes the SSL issue job finalization path only; existing already-issued certificates still need one manual/API HTTPS enable if they were issued before this fix.

---

## 2026-07-22 | `this commit` | Migration - Normalize WordPress URLs after Vesta import

**Summary:** Updated Vesta SQL import finalization to detect the imported WordPress `*_options` table and normalize `siteurl` and `home` to the scheme generated by ContainerCP for the migrated domain.
...
## 2026-07-22 | `this commit` | Migration - Report one-click Vesta job steps

**Summary:** Updated the one-click myVestaCP migration job to publish per-step status details as it advances through analysis, Site creation, file import, SQL import, final checks, and completion.

**Files changed:** `libs/api/ApiServer.cpp`, `CHANGELOG.md`

**User-visible behavior:** `/api/jobs?id=<id>` now reports meaningful step flags for Vesta migration jobs instead of leaving every migration step marked as skipped after completion.

**Validation:** Local build passed with `cmake --build build-db5 -j2`. Migration-focused doctests passed (`39` cases, `254` assertions). Frontend syntax passed for `web/pages/migration.js`; `git diff --check` and frontend baseline check passed.

**Known risks:** This changes job progress metadata only; migration execution and marker resume semantics are unchanged.
...
**Validation:** Clean configure and build passed for `containercp_tests`, `containercp`, and `containercpd` with no compiler diagnostics. Focused DB-4 tests passed, including artifact ID/path/filename policy, upload SQL policy, metadata redaction, exact import confirmation, manual-recovery diagnostics, provider export/import argv safety, and a real disposable MariaDB export/drop-table/import/row-restore cycle. Full doctest passed (`864` cases, `6166` assertions). CTest passed (`1/1`) when run independently. Frontend syntax, `node scripts/check-frontend-baseline.js`, `node scripts/test-database-drop-modal.js`, `node scripts/test-database-transfer-actions.js`, and `git diff --check` passed. Production deployment to `web2.softico.ua` via `/opt/containercp/scripts/update.sh` passed at commit `363a829`; target Site ID `13` / DB ID `13` reported `runtime_status=Running`, `connection_status=verified`, `credential_state=available`, `ownership_state=managed`, `can_export=true`, and `can_import=true`; export job `1` completed, authenticated artifact metadata/download worked without exposing SQL contents or secrets, raw upload staging succeeded, import job `2` completed with pre-import recovery and post-import access verification, artifact revoke returned success and subsequent metadata access returned `410`; target Site returned HTTP `200` through the proxy and target MariaDB container `site-13-db` remained healthy.
...
**Validation:** Clean configure passed. Clean build of `containercp_tests`, `containercp`, and `containercpd` passed with no compiler diagnostics. Focused DB-3 provider/lifecycle/API/Site-volume tests passed (`26` cases, `156` assertions), including a real disposable MariaDB drop/recreate/grant/login cycle with isolated non-`site-*` Compose project and cached `mariadb:lts`. Full doctest passed (`857` cases, `5940` assertions). CTest passed (`1/1`) when run independently. Frontend syntax, `node scripts/check-frontend-baseline.js`, `node scripts/test-database-drop-modal.js`, and `git diff --check` passed. Production deployment to `web2.softico.ua` via `/opt/containercp/scripts/update.sh` passed at commit `d3fd71c`; target Site ID `13` recreated database ID `12`, Create job `1` completed, Verify job `2` completed, API detail reported `runtime_status=Running`, `connection_status=verified`, `credential_state=available`, and `ownership_state=managed`; read-only physical counts confirmed target database/user/grant present and no global app-user privileges; target site returned HTTP `200`.
