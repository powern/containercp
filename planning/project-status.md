...
| Intermediate: Adminer server-side SSO handoff | ✅ | `libs/core/ServiceRegistry.cpp`, `libs/api/WebServer.*`, `libs/sqlconsole/*`, `tests/test_sql_console_session.cpp`, `tests/test_api.cpp` |
| Phase 7: Database GUI integration | ✅ | `web/pages/databases.js`, `tests/test_api.cpp`, `docs/development/sql-console.md`, `docs/api/API_REFERENCE.md` |
...
| DB-5: Database-Aware Site Backups | ✅ | BackupService/BackupJobService, DB-4-backed SQL payloads in backup archives, safe manifest metadata, async create/restore, download/remove, restore modes, and Backups GUI implemented; local build/doctest/CTest/frontend validation passed; VM validation on `test-gui-apache.local` / Site ID `13` passed after manifest detection fix `a4e11e8` |
...
| Sites | Stable | |
| Domains | Stable | |
| Databases | Active | DB-1 inventory, DB-2 GUI, DB-3 safe MariaDB lifecycle, DB-4 logical SQL export/import, and DB-5 database-aware backup integration implemented and VM-validated; Adminer, imported adoption, and multi-database support remain deferred |
| Users | Stable | |
| PHP Versions | Stable | |
...
| Current Epic | ARCH-009 Phase 3 accepted with residual integration risks; Phase 4 not started |
